...
def scrape_Google_Images(search_term: str, target_path='./pictureApp/static/pictures', img_count=5, picture_log={}):
    ...
    def fetch_image_urls(query: str, max_links_to_fetch: int, wd: webdriver, sleep_between_interactions: int = 1):
        ...
        while image_count < max_links_to_fetch:
            ...
            picture_log[len(
                picture_log)] = f"Found: {number_results} search results. Extracting links from {results_start}:{number_results}"
            ...
            for img in thumbnail_results[results_start:number_results]:
                ...
                if len(image_urls) >= max_links_to_fetch:
                    ...
                    picture_log[len(
                        picture_log)] = f"Found: {len(image_urls)} image links, done!"
                    break
            else:
                print("Found:", len(image_urls),
                      "image links, looking for more ...")
                img_url_count = len(image_urls)
                picture_log[len(
                    picture_log)] = f"Found: {img_url_count}image links, looking for more ..."
                time.sleep(30)
                return
    ...
    def persist_image(folder_path: str, url: str):
        ...
        try:
            ...
            file_path_alias = file_path[file_path.find("/static"):]
            picture_log[len(picture_log)
                        ] = f"SUCCESS - saved {url} - as {file_path_alias}"
        except Exception as e:
